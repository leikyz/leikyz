![Header](https://capsule-render.vercel.app/api?type=waving&height=200&color=gradient&text=Game%20Network%20Programmer&fontSize=50&fontAlign=50&fontAlignY=40&animation=fadeIn&section=header&reversal=false)
## About Me

I am a Game Developer with a deep passion for **low-level architecture, systems optimization, and multiplayer infrastructure**. 

---

### Core Expertise & Philosophy

* **Low-Level Network Programming:** Passionate about sockets, data serialization, and custom transport protocols and engine architecture.
* **Game Backend & Online Services:** Experienced in building server infrastructure and online features for reliable multiplayer sessions.
* **Networked Gameplay:** Comfortable building gameplay features like character controllers with client prediction and server reconciliation.
---

### 🛠️ Technical Stack

* **Languages:** C++ | Go | C# | SQL
* **Engines:** Unreal Engine 5 (C++ & Blueprints) | Unity (DOTS) | Custom Engine
